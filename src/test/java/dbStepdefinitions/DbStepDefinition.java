...
public class DbStepDefinition {
    ...
    public void kullanici_sutunundaki_verileri_okur_ve_istedigi_islemleri_yapar(String field) throws SQLException {
        ...
        System.out.println(resultSet.getRow());//kaçıncı satırda olduğumuzu verir

        //resultSet.next() bir sonraki objeye geçer ,sonraki eleman varsa true,yoksa false döndürür
        System.out.println(resultSet.next());//true
        System.out.println(resultSet.getObject(field).toString());

        //son objeye gider ve sonra resultSet.next() çalıştırırsak
        //bir sonraki eleman olmayacağı için bize false döner

        resultSet.last();
        System.out.println(resultSet.next());//false

        //Tüm listeyi yazdırmak istersek while loop ile birlikte resultSet.next()
        //kullanabiliriz.Cunku resultSet.next()bir sonraki obje var olduğu sürece
        //bize true dondurecek ve while loop calismya devam edecek
